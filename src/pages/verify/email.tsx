...
function VerifyPage() {
  ...
  const getUserVerifyChangeEmail = useGetUserVerifyChangeEmail(
    code as string,
    email as string
  )
  useEffect(() => {
    if (getUserVerifyChangeEmail.isSuccess) {
      toast.success('Verify Email Success')
      router.push('/profile')
    }
  }, [getUserVerifyChangeEmail.isSuccess])

  useEffect(() => {
    if (getUserVerifyChangeEmail.isError) {
      const errmsg = getUserVerifyChangeEmail.error as AxiosError<
        APIResponse<null>
      >
      toast.error(
  ...
  }, [getUserVerifyChangeEmail.isError])

  function Verify() {
    getUserVerifyChangeEmail.mutate()
  }
  return (
    <>
      <Head>
